    ...
    >
      <div className="flex flex-col gap-3 sm:flex-row">
        ...
        <label className="flex items-center gap-2 text-sm text-kot-ink">
          <span className="font-display uppercase tracking-wider text-kot-char">Sort</span>
          <select
            aria-label="Sort"
            className={selectClass}
            value={current("sort") || "surname"}
            onChange={(e) => setParam("sort", e.target.value === "surname" ? "" : e.target.value)}
          >
...
function Dropdown({
  ...
  return (
    <label className="flex items-center gap-2 text-sm text-kot-ink">
      <span className="font-display uppercase tracking-wider text-kot-char">{label}</span>
